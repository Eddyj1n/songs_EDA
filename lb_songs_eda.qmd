...
#### 2.1 Exploratory data analysis: Investigate temporal patterns across total songs I
...
```{r}
# # Plot time series of total songs sold at the weekly frequency
...
#   arrange(year_week)
# 
# hchart(songs_weekly_series_df, "line", hcaes(x = year_week, y = total_songs_sold)) %>%
#   hc_title(text = "Weekly Total Songs Sold Over Time") %>%
#   hc_xAxis(title = list(text = "Year-Week")) %>%
#   hc_yAxis(title = list(text = "Total Songs Sold")) %>%
#   hc_tooltip(valueDecimals = 0)

# # Plot time series of total songs sold at the monthly frequency
# songs_monthly_series_df <-cleaned_songs_df %>%
#   mutate(year_month = format(date, "%m-%Y")) %>%
#   group_by(year_month) %>%
#   summarize(total_songs_sold = sum(copies_sold))
# 
# hchart(songs_monthly_series_df, "line", hcaes(x = year_month, y = total_songs_sold)) %>%
#   hc_title(text = "Monthly Total Songs Sold Over Time") %>%
...
- Number of songs that sold above 2 million and the count to see if there is class imbalance. 
 Findings:  As the proportion of songs sold above 2 million only covers 6% of the data, recommend using regression approach. 
- Total count of songs released in each year in aggregate and by genre. 
 Findings:  Folktronica features releases the most songs by year consistent with the amount of copies sold 
- Total count of songs released by band in across all time to see if there are any data anomalies 
- Total count of unique bands to see if it's feasible to train a model at the band level
  Findings: There are 1390 unique bands and 7970 observations, where most bands have released less than 5 songs. Insufficient degrees of freedom to estimate a statisical model, pivot to genre level. 
...
Investigate if there are any temporal patterns/anomalies at the yearly levels.
Findings: The distributions of the continuous features are relatively stable across the years.
...
# Plot scatterplot and marginal distribtuion of target against continuous features using ggpubr 
for (vars in variables) {
  plot <- ggplot(cleaned_songs_df, aes(x = !!sym(vars), y = copies_sold, color = factor(genre))) +
    geom_point(alpha = 0.6) +
    scale_color_discrete(name = "Genre") +
    theme_minimal() +
    labs(x = vars, y = "Copies Sold", title = paste("Scatter Plot of", vars, "vs. Copies Sold"))
  
  plot_with_margin <- ggMarginal(plot, type = "density", fill = "lightblue", color = "lightblue")
